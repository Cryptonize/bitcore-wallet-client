...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.coin || 'bch', opts.network || 'livenet');
};
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.coin || 'bch', opts.network || 'livenet', opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromExtendedPrivateKey = function(xPrivKey, opts) {
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPrivateKey(opts.coin || 'bch', xPrivKey, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.coin || 'bch', opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPublicKey(opts.coin || 'bch', xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
};
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.coin || 'bch', opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, {
...
API.prototype.importFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts, cb) {
  ...
  log.debug('Importing from Extended Private Key');
  try {
    this.credentials = Credentials.fromExtendedPublicKey(opts.coin || 'bch', xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
  } catch (e) {
    log.info('xPriv error:', e);
    return cb(new Errors.INVALID_BACKUP);
  };
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  self._doGetRequest('/v2/feelevels/?coin=' + (coin || 'bch') + '&network=' + (network || 'livenet'), function(err, result) {
    if (err) return cb(err);
...
API.prototype.fetchPayPro = function(opts, cb) {
  ...
  PayPro.get({
    ...
    coin: this.credentials.coin || 'bch',
  }, function(err, paypro) {
    if (err)
      return cb(err);
